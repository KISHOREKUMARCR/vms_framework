...
(function () {
  ...
  // --------------------------------------------------------------------
  const cpuTemperatureElement = document.getElementById('cpuTemperature');
  const cpuTemperature = parseFloat(cpuTemperatureElement.value);
  const formattedCpuTemperature = cpuTemperature.toFixed(1); // Format with one decimal place
  ...
  const growthRadialChartEl = document.querySelector('#growthRadialChart'),
    growthRadialChartConfig = {
      ...
      series: [formattedCpuTemperature],
      labels: ['Temp']
    };
